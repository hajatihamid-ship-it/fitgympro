...
import { sanitizeHTML, applySiteSettings, showToast } from './utils/dom';
import { initRouter, navigateTo } from './router';
import type { User, UserData, StorePlan } from './types';
...
const seedInitialUsers = async () => {
    ...
    if (users.length === 0) {
        ...
        const userWorkoutData: UserData = {
            step1: {
...
const showDashboard = async () => {
    ...
    if (!appContainer || !currentUser) {
        // This case should be handled by the router guard, but as a fallback:
        await handleLogout();
        return;
    }
...
const initPwaInstall = () => {
    window.addEventListener('beforeinstallprompt', (e) => {
        ...
        if (installButton) {
            ...
            (installButton as HTMLElement).style.display = 'inline-flex';
        }
    });
